...
int main(int argc, char **argv) {
	...
	render.setPostProcessSetup(
    	[&tonemap, &chromatic, &greyscale, &vignette, &grain, &grade, &dither,
		 &solarize, &sabattier, &emboss, &sobel, &speedLines, &highlight, &segmentation](vvppl::PostProcessing &pp) {
			...
			// Start the new effects switched off, the panel turns them on.
			solarize->threshold = 1.0F;
			sabattier->strength = 0.0F;
			emboss->strength = 0.0F;
			sobel->strength = 0.0F;
			speedLines->intensity = 0.0F;
			highlight->strength = 0.0F;
			segmentation->strength = 0.0F;
    });
	...
	engine.world().get<vve::GuiSystem>().draw([&frame, &activeRenderer, &cameraController, &renderFps,
		...
		if (sabattier && ImGui::CollapsingHeader("Sabattier")) {
		...
		if (speedLines && ImGui::CollapsingHeader("Speed lines")) {
		...
		if (highlight && ImGui::CollapsingHeader("Highlight")) {
		...
		if (segmentation && ImGui::CollapsingHeader("Segmentation")) {
