...
namespace vve {
    ...
    void createDescriptorSetsRT(std::vector<VkDescriptorSet>& descriptorSets, VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout,
        VkAccelerationStructureKHR tlas, DeviceBuffer<Vertex>* vertexBuffer, DeviceBuffer<uint32_t>* indexBuffer, std::vector<HostBuffer<Instance>*> instanceBuffers, VkDevice device);
}