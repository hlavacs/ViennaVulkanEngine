...
namespace ve {
    ...
    *
    */
    VECamera *VESceneManager::createCamera(std::string cameraName, VECamera::veCameraType type, VESceneNode *parent,
                                           glm::mat4 transf) {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    *
    */
    VELight *VESceneManager::createLight(std::string lightName, VELight::veLightType type, VESceneNode *parent,
                                         glm::mat4 transf) {
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    *
    */
    void VESceneManager::updateSceneNodes(uint32_t
                                          imageIndex) {
        std::lock_guard<std::mutex> lock(m_mutex);

        m_lights.

                clear();                                                //light vector will be created dynamically

        updateSceneNodes2(getRoot(), glm::mat4(1.0f), imageIndex

        );

        while (m_updateFutures.

                size()

               > 0) {                            //gets all futures from the threads and waits for them
            m_updateFutures.
        ...
        for (
            auto list
                : m_memoryBlockMap) {                            //update all UBO buffers, i.e. copy them to the GPU
            vh::vhMemBlockUpdateBlockList(list
                                                  .second, imageIndex);
        }
    }
    ...
    void VESceneManager::updateSceneNodes2(VESceneNode *pNode, glm::mat4 parentWorldMatrix, uint32_t imageIndex) {
        ...
        if (pNode->m_children.size() > 0) {
            ...
            if (tp->threadCount() > 1 && pNode->m_children.size() > granularity) {
                ...
                uint32_t
                        numChildrenPerThread = (uint32_t)
                                                       pNode->m_children.size() / numThreads;

                uint32_t startIdx, endIdx;
                for (uint32_t k = 0; k < numThreads; k++) {
                    startIdx = k *
                               numChildrenPerThread;                                                                            //start index for parallel run
                    endIdx = k == numThreads - 1 ? (uint32_t) pNode->m_children.size() - 1 :
                             (k + 1) * numChildrenPerThread -
                             1;    //end index
                ...
                updateSceneNodes3(pNode->m_children, worldMatrix, 0, (uint32_t)
                                                                             pNode->m_children.size() - 1,
...
*/
    void
    VESceneManager::updateSceneNodes3(std::vector<VESceneNode *> &children, glm::mat4 worldMatrix, uint32_t startIdx,
                                      uint32_t endIdx, uint32_t imageIndex) {
    ...
    void VESceneManager::notifyEventListeners(VESceneNode *pNode) {
        ...
        for (auto listener : *listeners) {                                            //go through them and call onSceneNodeDeleted()
            if (listener->onSceneNodeDeleted(
                    event)) {                                //if the listener answers with true, it wants to be destroyed
    ...
    void VESceneManager::deleteSceneNodeAndChildren2(VESceneNode *pNode) {
        ...
        if (pNode->getNodeType() == VESceneNode::VE_NODE_TYPE_SCENEOBJECT) {
            ...
            if (pObject->m_memoryHandle.pMemBlock !=
                nullptr) {                                //remove it from the UBO list
