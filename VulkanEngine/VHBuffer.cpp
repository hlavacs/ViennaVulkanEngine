...
namespace vh {
    ...
    */
    VkResult vhBufCreateBuffer(VmaAllocator
        ...
        allocInfo.
                usage = vmaUsage;                    //VMA_MEMORY_USAGE_GPU_ONLY;
...
*/
    VkResult vhBufCreateDepthResources(VkDevice device, VmaAllocator allocator, VkQueue graphicsQueue,
        ...
        VHCHECKRESULT(
                vhBufCreateImageView(device, *depthImage, depthFormat, VK_IMAGE_VIEW_TYPE_2D, 1,
...
*/
    VkResult vhBufCreateOffscreenResources(VkDevice device, VmaAllocator allocator, VkQueue graphicsQueue,
        ...
        VHCHECKRESULT(
                vh::vhBufCreateImageView(device, *image, format, VK_IMAGE_VIEW_TYPE_2D, 1, VK_IMAGE_ASPECT_COLOR_BIT,
                                         colorImageView));
...
*/
    VkResult vhBufCopyBufferToImage(VkDevice device, VkQueue graphicsQueue, VkCommandPool commandPool,
        ...
        vkCmdCopyBufferToImage(commandBuffer, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, (uint32_t)
...
*/
    VkResult vhBufCopyImageToBuffer(VkDevice device, VkQueue graphicsQueue, VkCommandPool commandPool,
        ...
        vkCmdCopyImageToBuffer(commandBuffer, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, buffer, (uint32_t)
                regions.size(), regions.data());
    ...
    vhBufCreateTextureImage(VkDevice device, VmaAllocator allocator, VkQueue graphicsQueue, VkCommandPool commandPool,
        ...
        for (uint32_t face = 0; face < imageData.size(); face++) {
            for (uint32_t level = 0; level < mipLevels; level++) {
                ...
                offset += (uint32_t)
                        imageData[face].imageSize;
            }
        }


        VHCHECKRESULT(
                vhBufCreateImage(allocator, imageData[0].texWidth, imageData[0].texHeight, 1,
...
*/
    VkResult vhBufCreateFramebuffers(VkDevice device,
        ...
        uint32_t
                loops = (uint32_t)
                std::max(imageViews.size(), depthImageViews.size());
...
*/
    VkResult vhBufCreateFramebuffersOffscreen(VkDevice device,
        ...
        uint32_t
                loops = (uint32_t)
                std::max(positionImageViews.size(), depthImageViews.size());
        frameBuffers.resize(loops);
...
*/
    ...
    vhBufCreateIndexBuffer(VkDevice device, VmaAllocator allocator, VkQueue graphicsQueue, VkCommandPool commandPool,
        ...
        void *data;
        VHCHECKRESULT(vmaMapMemory(allocator, stagingBufferAllocation, &data));
        memcpy(data, indices.data(), (size_t)
                bufferSize);
        vmaUnmapMemory(allocator, stagingBufferAllocation);
