...
namespace ve {
    ...
    VESubrenderRT_DN::VESubrenderRT_DN():m_enableShadows(VK_TRUE) {};
	...
	*/
	void VESubrenderRT_DN::initSubrenderer() {
        ...
        vh::vhPipeCreateGraphicsPipelineLayout(getRendererRTPointer()->getDevice(),
			{ perObjectLayout, perObjectLayout, m_descriptorSetLayoutOutput, m_descriptorSetLayoutAS, m_descriptorSetLayoutGeometry, m_descriptorSetLayoutObjectUBOs, m_descriptorSetLayoutResources },
    ...
    void VESubrenderRT_DN::createRaytracingDescriptorSets()
    {
        if(m_entities.size())
        {
            ...
            VkResult        code = vkAllocateCommandBuffers(getRendererRTPointer()->getDevice(), &commandBufferAllocateInfo, &commandBuffer);
            if(code != VK_SUCCESS)
            {
            ...
            VkResult result = vh::vhCmdEndSingleTimeCommands(getRendererRTPointer()->getDevice(), getRendererRTPointer()->getGraphicsQueue(), getRendererRTPointer()->getCommandPool(), commandBuffer,
                VK_NULL_HANDLE,
                VK_NULL_HANDLE,
                VK_NULL_HANDLE
            );
        ...
        vh::vhRenderCreateDescriptorSetLayout(getRendererRTPointer()->getDevice(), 	//binding 0...array, binding 1...array
            { m_resourceArrayLength,						m_resourceArrayLength },
            { VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,	VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER },
            { VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV,					VK_SHADER_STAGE_CLOSEST_HIT_BIT_NV },
            &m_descriptorSetLayoutResources);
        // Acceleration structure and geometry are the same for all swapchains
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), 1, m_descriptorSetLayoutAS, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsAS);
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), 1, m_descriptorSetLayoutGeometry, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsGeometry);
        // Output image and UBOPerEntity changes from Swapchain to Swapchain, for each swapchain create own descriptor set
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), getRendererRTPointer()->getSwapChainNumber(), m_descriptorSetLayoutOutput, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsOutput);
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), getRendererRTPointer()->getSwapChainNumber(), m_descriptorSetLayoutObjectUBOs, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsUBOs);
    }
	...
	void VESubrenderRT_DN::createRTGraphicsPipeline()
	{
		...
		VkShaderModule rayGenModule = vh::vhPipeCreateShaderModule(getRendererRTPointer()->getDevice(), rayGenShaderCode);
        ...
        VkShaderModule shadowMissModule = vh::vhPipeCreateShaderModule(getRendererRTPointer()->getDevice(), shadowMissShaderCode);
		...
		VkShaderModule closestHitModule = vh::vhPipeCreateShaderModule(getRendererRTPointer()->getDevice(), closestHitShaderCode);
        ...
        vkDestroyShaderModule(getRendererRTPointer()->getDevice(), shadowMissModule, nullptr);
    ...
    void VESubrenderRT_DN::createShaderBindingTable()
    {
        ...
        nv_helpers_vk::createBuffer(getRendererRTPointer()->getPhysicalDevice(), getRendererRTPointer()->getDevice(), shaderBindingTableSize,
	...
	*/
    ...
    void VESubrenderRT_DN::UpdateRTDescriptorSets()
    {
        ...
        for(auto &entity : m_entities)
        {
            ...
            vmaGetAllocationInfo(getRendererRTPointer()->getVmaAllocator(), entity->m_pMesh->m_vertexBufferAllocation, &info);
            ...
            vmaGetAllocationInfo(getRendererRTPointer()->getVmaAllocator(), entity->m_pMesh->m_indexBufferAllocation, &info);
        ...
        if(m_entities.size())
        {
            ...
            vkUpdateDescriptorSets(getRendererRTPointer()->getDevice(), static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
    ...
    *
    * IMPORTANT: this function is a copy of VESubrendererRT. This function must be changed to load ressources of all objects 
    * at once. Now each objects has its own descriptor set. TODO: for all object create one descriptor set with all ressources.
    *
    *
	...
	*/
	void VESubrenderRT_DN::addMaps(VEEntity* pEntity, std::vector<VkDescriptorImageInfo>& newMaps) {
		...
		vh::vhRenderUpdateDescriptorSetMaps(getRendererRTPointer()->getDevice(),					//update the descriptor that holds the map array
