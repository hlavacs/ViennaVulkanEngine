 ...
 */
    void VERendererDeferred::initRenderer() {
        ...
        if (vh::vhDevPickPhysicalDevice(getEnginePointer()->getInstance(), m_surface,
            ...
            vh::vhDevCreateLogicalDevice(
                    getEnginePointer()->getInstance(), m_physicalDevice, m_surface,
        ...
        for (uint32_t i = 0; i < m_swapChainImageViews.size(); i++) {
            for (uint32_t j = 0; j < NUM_SHADOW_CASCADE;
                 j++) { // point light has 6 shadow maps, thats the max
        ...
        vh::vhRenderCreateDescriptorSets(m_device, (uint32_t)
 ...
 */
    ...
    VERendererDeferred::secondaryCmdBuf_t VERendererDeferred::recordRenderpass(
            VkRenderPass *pRenderPass, std::vector<VESubrender *> subRenderers,
            VkFramebuffer *pFrameBuffer, uint32_t imageIndex, uint32_t numPass,
