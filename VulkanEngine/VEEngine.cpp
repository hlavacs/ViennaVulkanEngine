...
namespace ve {
    ...
    */
    void VEEngine::initEngine() {
        ...
        m_pWindow->initWindow(1080, 720);    //inittialize the window
        ...
        if (m_debug)
            vh::vhSetupDebugCallback(m_instance,
                                     &callback);                //create a debug callback for printing debug information
        ...
        for (uint32_t i = veEvent::VE_EVENT_NONE;
             i < veEvent::VE_EVENT_LAST; i++) {    //initialize the event listeners lists
    ...
    */
    void VEEngine::callListeners(double dt, veEvent event, std::vector<VEEventListener *> *list) {
        ...
        if (m_threadPool->threadCount() > 1 && list->size() > granularity) {
            ...
            uint32_t
                    numListenerPerThread = (uint32_t)
                                                   list->size() / numThreads;
            ...
            if (list->size() > 0)
                callListeners2(dt, event, list, 0, (uint32_t)
                                                           list->size() - 1);
        }
    }
    ...
    *
    */
    void VEEngine::callListeners2(double dt, veEvent event, std::vector<VEEventListener *> *list, uint32_t startIdx,
                                  uint32_t endIdx) {
    ...
    void VEEngine::run() {
        ...
        while (!m_end_running) {
            if (m_loopCount > 0) {
                ...
                //m_AcceptableLevel1 = vh::vhAverage(m_dt < 1.0 / 60 ? 1.0 : 0.0, m_AcceptableLevel1);
                //m_AcceptableLevel2 = vh::vhAverage(m_dt<1.0/60?1.0:0.0, m_AcceptableLevel2);
                t_prev = vh::vhTimeNow();
            }
            m_AcceptableLevel1 =
    ...
    */
    ...
    void VEEngine::loadLevel(uint32_t
                             numLevel) {
