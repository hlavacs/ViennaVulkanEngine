...
namespace ve {
	...
	*/
	void VESubrenderRT_DN::initSubrenderer() {
        ...
        vh::vhPipeCreateGraphicsPipelineLayout(getRendererRTPointer()->getDevice(),
			{ perObjectLayout, perObjectLayout, m_descriptorSetLayoutOutput, m_descriptorSetLayoutAS, m_descriptorSetLayoutGeometry, perObjectLayout, m_descriptorSetLayoutResources },
			{}, &m_pipelineLayout);
    ...
    void VESubrenderRT_DN::createRaytracingDescriptorSets()
    {
        ...
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), getRendererRTPointer()->getSwapChainNumber(), m_descriptorSetLayoutAS, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsAS);
        vh::vhRenderCreateDescriptorSets(getRendererRTPointer()->getDevice(), getRendererRTPointer()->getSwapChainNumber(), m_descriptorSetLayoutGeometry, getRendererRTPointer()->getDescriptorPool(), m_descriptorSetsGeometry);
	...
	*/
	void VESubrenderRT_DN::bindDescriptorSetsPerEntity(VkCommandBuffer commandBuffer, uint32_t imageIndex, VEEntity* entity) {
        ...
        sets.push_back(entity->m_memoryHandle.pMemBlock->descriptorSets[imageIndex]);
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_RAY_TRACING_NV, m_pipelineLayout,
			5, (uint32_t)sets.size(), sets.data(), 1, &offset);
        ...
        if(m_entities.size())
        {
            for(size_t i = 0; i < m_descriptorSetsGeometry.size(); i++)
            {
                vh::vhRenderUpdateDescriptorSet(getRendererRTPointer()->getDevice(), m_descriptorSetsGeometry[i],
                    { VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , VK_DESCRIPTOR_TYPE_STORAGE_BUFFER },
                    { m_entities[0]->m_pMesh->m_vertexBuffer , m_entities[0]->m_pMesh->m_indexBuffer },
	...
	*
	*/
	void VESubrenderRT_DN::drawEntity(VkCommandBuffer commandBuffer, uint32_t imageIndex, VEEntity* entity) {
