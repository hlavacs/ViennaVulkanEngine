...
#	/**
    ...
    */
    VkResult vhRenderCreateDescriptorSets(VkDevice device,
        ...
        descriptorSets.resize(size +
                              numberDesc);                                        //resize so we can append new sets to the vector
        ...
        for (uint32_t i = 0; i < uniformBuffers.size(); i++) {
            ...
            if (uniformBuffers[i] != VK_NULL_HANDLE) {
                ...
                descriptorWrites[i].descriptorType =
                        descriptorTypes.size() > i ? descriptorTypes[i] : VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
                descriptorWrites[i].pImageInfo = imageInfos[i].data();
                descriptorWrites[i].descriptorCount = (uint32_t)
                        textureImageViews[i].size();
            } else {
    ...
    */
    VkResult vhRenderBeginRenderPass(VkCommandBuffer
        ...
        return
                vhRenderBeginRenderPass(commandBuffer, renderPass, frameBuffer, clearValues, extent, subPassContents
                );
    }
...
*/
    VkResult vhPipeCreateGraphicsPipelineLayout(VkDevice device,
        ...
        pipelineLayoutInfo.setLayoutCount = (uint32_t)
                descriptorSetLayouts.size();
        pipelineLayoutInfo.pSetLayouts = descriptorSetLayouts.data();
        if (pushConstantRanges.size() > 0) {
            pipelineLayoutInfo.pushConstantRangeCount = (uint32_t)
...
*/
    VkResult vhPipeCreateGraphicsPipeline(VkDevice device,
        ...
        dynamicState.dynamicStateCount = (uint32_t)
                dynamicStates.size();
