...
namespace vve {
	...
	*/
	...
	protected:
		...
		*/
		void construct( uint8_t* startptr, VeIndex size ) {
			...
			for (uint32_t i = 0; i < size; ++i) {		//go through all entries
				uint8_t* ptr = startptr + (uint64_t)i * m_entrySize;	//point to the entry address
		...
		*/
		void copy(uint8_t* start_dst, VeIndex entry_size_dst, uint8_t* start_src, VeIndex entry_size_src, VeIndex size_src) {
			if (m_memcpy && entry_size_dst == entry_size_src) {
				memcpy(m_startptr, start_src, (uint64_t)size_src * entry_size_src);
				return;
			}

			for (uint32_t i = 0; i < size_src; ++i) {
				uint8_t* ptrd = start_dst + (uint64_t)i * entry_size_dst;
				uint8_t* ptrs = start_src + (uint64_t)i * entry_size_src;
				*(T*)ptrd = *(T*)ptrs;
			}
			m_size = size_src;
		}
		...
		*/
		void setNewCapacity( VeIndex newcapacity) {
			...
			uint8_t* newmemptr = new uint8_t[newcapacity * (uint64_t)m_entrySize + m_alignment];
	...
	public:
		...
		*/
		...
		iterator end() { return iterator( m_startptr + (uint64_t)m_size * m_entrySize, m_entrySize); }	///<end for iterator
		const_iterator begin() const { return const_iterator(m_startptr, m_entrySize); }		///<const iterator begin
		const_iterator end() const { return const_iterator(m_startptr + (uint64_t)m_size * m_entrySize, m_entrySize);  } ///<const iterator end

		void resize(VeIndex new_size);			///<resize the size of the vector
	...
	*/
	template<typename T> inline VeVector<T>::VeVector(bool memcopy, VeIndex align, VeIndex capacity ) : m_size(0) {
		...
		m_memptr	= new uint8_t[(uint64_t)m_capacity * m_entrySize + m_alignment];
	...
	*/
	...
	template<typename T> inline void VeVector<T>::resize(VeIndex new_size) {
		...
		construct(m_startptr + (uint64_t)m_size * m_entrySize, new_size - m_size);
		m_size = new_size;
	}
	...
	*/
	template<typename T> inline void VeVector<T>::emplace_back(T &entry) {
		...
		uint8_t* ptr = m_startptr + (uint64_t)m_size * m_entrySize;
	...
	*/
	template<typename T> inline void VeVector<T>::swap(VeIndex a, VeIndex b) {
		...
		uint8_t* ptra = m_startptr + (uint64_t)a * m_entrySize;
		uint8_t* ptrb = m_startptr + (uint64_t)b * m_entrySize;
