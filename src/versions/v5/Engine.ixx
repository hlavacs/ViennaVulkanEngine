...
/// @file
/// @brief Small v5 runtime facade: SDL windows, input, graphs, and stub subsystems.

export namespace vve::v5 {

	struct TaskHandleTag {};								///< v5 CPU task graph node handle tag.

	using TaskHandle = TypedHandle<TaskHandleTag>;	///< v5 CPU task graph node handle.
...
export namespace vve::v5 {
	...
	/// @brief Educational v5 engine shell with SDL windows and lightweight subsystems.
	class Engine {
	public:
		Engine();
	...
	inline std::uint32_t Engine::versionMajor() const { return 5; }
	...
	inline std::string_view Engine::versionName() const { return "v5"; }
	...
	/// @brief Builds a v5 engine from typed options in any order.
	...
	inline auto Engine::writeDebugGraphs(const std::filesystem::path &directory) const	-> std::expected<void, Error>{
		...
		if (const auto result = tasks_.writeJson(task_path, "task_graph", "v5 task graph"); !result) { return result; }
		if (const auto result = render_graph_.writeJson(render_path, "render_graph", "v5 render graph"); !result) {
			return result;
		}
		...
		for (const auto &window : window_system_.snapshot()) {
			...
			const auto name = "v5 render graph window=" + window.id + " renderer=" + renderer->id.value;
