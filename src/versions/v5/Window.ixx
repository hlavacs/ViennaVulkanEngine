...
export namespace vve::v5 {
   ...
   class InputState {
   public:
      VVE_V5_API void beginFrame();
      VVE_V5_API void holdKey(std::int32_t keycode);
      VVE_V5_API void pressKey(std::int32_t keycode);
      VVE_V5_API void releaseKey(std::int32_t keycode);
      VVE_V5_API void setMousePosition(WindowHandle window, Vec2 position);
      VVE_V5_API void addMouseDelta(WindowHandle window, Vec2 delta);
      VVE_V5_API void addMouseWheelDelta(WindowHandle window, Vec2 delta);

      [[nodiscard]] VVE_V5_API bool isKeyDown(std::int32_t keycode) const;
      [[nodiscard]] VVE_V5_API bool wasKeyPressed(std::int32_t keycode) const;
      [[nodiscard]] VVE_V5_API bool wasKeyReleased(std::int32_t keycode) const;
      [[nodiscard]] VVE_V5_API std::optional<Vec2> mousePosition(WindowHandle window) const;
      [[nodiscard]] VVE_V5_API Vec2 mouseDelta(WindowHandle window) const;
      [[nodiscard]] VVE_V5_API Vec2 mouseWheelDelta(WindowHandle window) const;

   private:
      [[nodiscard]] static VVE_V5_API std::int32_t normalizeKey(std::int32_t keycode);
   ...
   public:
      ...
      [[nodiscard]] VVE_V5_API SDL_WindowID sdlId() const noexcept;
      [[nodiscard]] VVE_V5_API WindowInfo &info() noexcept;
      [[nodiscard]] VVE_V5_API const WindowInfo &info() const noexcept;
      [[nodiscard]] VVE_V5_API WindowHandle handle() const noexcept;
      [[nodiscard]] VVE_V5_API std::string_view id() const noexcept;
      [[nodiscard]] VVE_V5_API std::string_view title() const noexcept;
      [[nodiscard]] VVE_V5_API PixelExtent extent() const noexcept;
      [[nodiscard]] VVE_V5_API RendererId rendererId() const;
      [[nodiscard]] VVE_V5_API std::optional<Entity> camera() const;
      [[nodiscard]] VVE_V5_API bool focused() const noexcept;
      [[nodiscard]] VVE_V5_API bool minimized() const noexcept;
      [[nodiscard]] VVE_V5_API bool shouldClose() const noexcept;

   private:
      VVE_V5_API void reset() noexcept;
...
export namespace vve::v5 {
   ...
   VVE_V5_API void InputState::beginFrame() {
      keys_pressed_.clear();
   ...
   VVE_V5_API void InputState::pressKey(std::int32_t keycode) {
      const auto key = normalizeKey(keycode);
      if (!keys_down_.contains(key)) { keys_pressed_.insert(key); }
      keys_down_.insert(key);
   }

   VVE_V5_API void InputState::releaseKey(std::int32_t keycode) {
      const auto key = normalizeKey(keycode);
   ...
   VVE_V5_API void InputState::addMouseDelta(WindowHandle window, Vec2 delta) {
      const auto [it, _] = mouse_delta_.try_emplace(window, Vec2{zero(), zero()});
      it->second = math::add(it->second, delta);
   }

   VVE_V5_API void InputState::addMouseWheelDelta(WindowHandle window, Vec2 delta) {
   ...
   VVE_V5_API bool InputState::wasKeyPressed(std::int32_t keycode) const {
      return keys_pressed_.contains(normalizeKey(keycode));
   }

   VVE_V5_API bool InputState::wasKeyReleased(std::int32_t keycode) const {
      return keys_released_.contains(normalizeKey(keycode));
   }

   VVE_V5_API std::optional<Vec2> InputState::mousePosition(WindowHandle window) const {
      const auto it = mouse_position_.find(window);
      return it == mouse_position_.end() ? std::optional<Vec2>{} : std::optional<Vec2>{it->second};
   }

   VVE_V5_API Vec2 InputState::mouseDelta(WindowHandle window) const {
      const auto it = mouse_delta_.find(window);
      return it == mouse_delta_.end() ? Vec2{} : it->second;
   }

   VVE_V5_API Vec2 InputState::mouseWheelDelta(WindowHandle window) const {
      const auto it = mouse_wheel_delta_.find(window);
      return it == mouse_wheel_delta_.end() ? Vec2{} : it->second;
   }

   VVE_V5_API std::int32_t InputState::normalizeKey(std::int32_t keycode) {
   ...
   VVE_V5_API void Window::reset() noexcept {
      if (window_ != nullptr) {
   ...
   VVE_V5_API std::expected<void, Error> WindowSystem::init(const Windows &windows) {
   ...
   VVE_V5_API std::expected<void, Error> WindowSystem::poll() {
   ...
   VVE_V5_API Vector<WindowInfo> WindowSystem::snapshot() const {
   ...
   VVE_V5_API Vector<std::reference_wrapper<Window>> WindowSystem::windows() {
   ...
   VVE_V5_API Vector<std::reference_wrapper<const Window>> WindowSystem::windows() const {
   ...
   VVE_V5_API std::expected<void, Error> WindowSystem::setWindowCamera(WindowHandle window, Entity camera) {
      return editWindow(window, [camera](Window &selected) { selected.info().camera = camera; });
   }

   VVE_V5_API std::expected<void, Error> WindowSystem::setWindowCamera(std::string_view id, Entity camera) {
      return editWindow(id, [camera](Window &selected) { selected.info().camera = camera; });
   }

   VVE_V5_API std::expected<void, Error> WindowSystem::clearWindowCamera(WindowHandle window) {
      return editWindow(window, [](Window &selected) { selected.info().camera.reset(); });
   }

   VVE_V5_API std::expected<void, Error> WindowSystem::clearWindowCamera(std::string_view id) {
   ...
   VVE_V5_API std::expected<void, Error> WindowSystem::setActiveCamera(Entity camera) {
      if (impl_->windows.empty()) { return std::unexpected(Error::missing_object); }
      for (auto &window : impl_->windows) { window.info().camera = camera; }
      return {};
   }

   VVE_V5_API std::optional<Entity> WindowSystem::activeCamera() const {
   ...
   VVE_V5_API bool WindowSystem::anyShouldClose() const {
