...
export module VEEngine.V5:Shaders;
import std;
export import :Types;
...
export namespace vve::v5 {
	...
	class ShaderSystem {
	public:
		[[nodiscard]] VVE_V5_API std::expected<ShaderHandle, Error> addShader(ObjectName name,
																										Vector<ShaderStage> stages);
		[[nodiscard]] VVE_V5_API std::expected<ShaderHandle, Error>
		compileAndReflect(const std::filesystem::path &source, Vector<std::string> entry_points);
		[[nodiscard]] VVE_V5_API auto containsShader(ShaderHandle handle) const											-> bool;
		[[nodiscard]] VVE_V5_API auto shaderName(ShaderHandle handle) const												-> std::expected<ObjectName, Error>;
		[[nodiscard]] VVE_V5_API auto shaderStageCount(ShaderHandle handle) const										-> std::expected<std::size_t, Error>;
		[[nodiscard]] VVE_V5_API std::expected<std::size_t, Error> spirvWordCount(ShaderHandle handle,
																											ShaderStage stage) const;
		[[nodiscard]] VVE_V5_API std::expected<Vector<std::uint32_t>, Error> stageSpirv(ShaderHandle handle,
																													ShaderStage stage) const;
		[[nodiscard]] VVE_V5_API std::expected<bool, Error> hasReflectedBinding(ShaderHandle handle,
																										std::string_view name) const;
		[[nodiscard]] VVE_V5_API std::expected<bool, Error> hasReflectedType(ShaderHandle handle,
																									std::string_view name) const;
		[[nodiscard]] VVE_V5_API std::expected<Vector<ShaderBindingReflection>, Error>
		reflectedBindings(ShaderHandle handle) const;
		[[nodiscard]] VVE_V5_API std::expected<Vector<ShaderEntryPointReflection>, Error>
...
export namespace vve::v5 {
	...
	VVE_V5_API auto ShaderSystem::addShader(ObjectName name, Vector<ShaderStage> stages)									-> std::expected<ShaderHandle, Error>{
	...
	VVE_V5_API bool ShaderSystem::containsShader(ShaderHandle handle) const { return shaders_.contains(handle); }

	/// @brief Returns the shader name.
	VVE_V5_API auto ShaderSystem::shaderName(ShaderHandle handle) const															-> std::expected<ObjectName, Error>{
	...
	VVE_V5_API auto ShaderSystem::shaderStageCount(ShaderHandle handle) const													-> std::expected<std::size_t, Error>{
		const auto shader = shaders_.find(handle);
		if (shader == shaders_.end()) { return std::unexpected(Error::missing_object); }
	...
	VVE_V5_API std::expected<ShaderHandle, Error> ShaderSystem::compileAndReflect(const std::filesystem::path &source,
	...
	VVE_V5_API auto ShaderSystem::spirvWordCount(ShaderHandle handle, ShaderStage stage) const							-> std::expected<std::size_t, Error>{
	...
	VVE_V5_API std::expected<Vector<std::uint32_t>, Error> ShaderSystem::stageSpirv(ShaderHandle handle,
																								ShaderStage stage) const {
		const auto *shader = find(handle);
		if (shader == nullptr) { return std::unexpected(Error::missing_object); }
		const auto it = std::ranges::find_if(shader->binaries, [stage](const auto &binary) {
	...
	VVE_V5_API auto ShaderSystem::hasReflectedBinding(ShaderHandle handle, std::string_view name) const				-> std::expected<bool, Error>{
		const auto *shader = find(handle);
		if (shader == nullptr) { return std::unexpected(Error::missing_object); }
	...
	VVE_V5_API auto ShaderSystem::hasReflectedType(ShaderHandle handle, std::string_view name) const					-> std::expected<bool, Error>{
		const auto *shader = find(handle);
		if (shader == nullptr) { return std::unexpected(Error::missing_object); }
	...
	VVE_V5_API auto ShaderSystem::reflectedBindings(ShaderHandle handle) const													-> std::expected<Vector<ShaderBindingReflection>, Error>{
		const auto *shader = find(handle);
		if (shader == nullptr) { return std::unexpected(Error::missing_object); }
		return shader->reflection.bindings;
	}

	VVE_V5_API auto ShaderSystem::reflectedEntryPoints(ShaderHandle handle) const													-> std::expected<Vector<ShaderEntryPointReflection>, Error>{
		const auto *shader = find(handle);
		if (shader == nullptr) { return std::unexpected(Error::missing_object); }
