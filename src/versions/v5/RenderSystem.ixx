...
export namespace vve::v5 {
   ...
   public:
      ...
      [[nodiscard]] std::expected<RenderGraph, Error> buildRenderGraph(std::span<const RenderPassContract> passes) const;
      ...
      [[nodiscard]] std::expected<ObjectName, Error> resourceName(RenderResourceHandle handle) const;
      [[nodiscard]] std::expected<RenderResourceKind, Error> resourceKind(RenderResourceHandle handle) const;
      [[nodiscard]] std::expected<ObjectName, Error> functionName(RenderFunctionHandle handle) const;
...
namespace vve::v5 {
   ...
   inline std::expected<RenderGraph, Error> RenderSystem::buildRenderGraph(std::span<const RenderPassContract> passes) const {
   ...
   inline std::expected<ObjectName, Error> RenderSystem::resourceName(RenderResourceHandle handle) const {
   ...
   inline std::expected<RenderResourceKind, Error> RenderSystem::resourceKind(RenderResourceHandle handle) const {
      const auto *resource = find(handle);
      return resource == nullptr ? std::unexpected(Error::missing_object) :
                                   std::expected<RenderResourceKind, Error>{resource->kind};
   }

   inline std::expected<ObjectName, Error> RenderSystem::functionName(RenderFunctionHandle handle) const {
   ...
   inline std::expected<void, Error> RenderSystem::addPlane(Vec2 half_extent, LinearColor color, Transform transform) {
