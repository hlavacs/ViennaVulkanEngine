...
export module VEEngine.V5:Vulkan;
import std;
import :VulkanLow;
import :Window;
...
export namespace vve::v5::vh {
   ...
   struct FrameTarget {
      ...
      vk::raii::DeviceMemory spot_shadow_depth_memory{nullptr}; ///< Memory backing the spot shadow depth image.
      vk::raii::Image spot_shadow_depth_image{nullptr};   ///< Spot shadow depth image.
      vk::raii::ImageView spot_shadow_depth_view{nullptr}; ///< Spot shadow depth image view.
   ...
   struct SceneDebugSample {
      ...
      std::array<float, 4> spot_light_clip{};              ///< GPU-computed spot light clip position.
      ...
      std::array<float, 3> spot_light_ndc{};               ///< GPU-computed spot light NDC position.
      float spot_light_depth{};                            ///< GPU-computed spot light depth.
      ...
      float sampled_spot_shadow_depth{};                   ///< Spot shadow depth sampled by the shader.
   ...
   public:
      ...
      [[nodiscard]] VVE_V5_API std::expected<void, Error> prepare(WindowSystem &windows);
      [[nodiscard]] VVE_V5_API std::expected<void, Error> renderClear(const std::array<float, 4> &color);
      [[nodiscard]] VVE_V5_API std::expected<void, Error>
      ...
      [[nodiscard]] VVE_V5_API std::size_t targetCount() const;
      [[nodiscard]] VVE_V5_API bool ready() const;
      [[nodiscard]] VVE_V5_API std::uint64_t presentedFrameCount() const;
      [[nodiscard]] VVE_V5_API std::uint64_t triangleDrawCount() const;
      [[nodiscard]] VVE_V5_API std::uint32_t triangleVertexCount() const;
      [[nodiscard]] VVE_V5_API std::uint64_t sceneUploadCount() const;
      [[nodiscard]] VVE_V5_API std::uint64_t sceneMeshDrawCount() const;
      [[nodiscard]] VVE_V5_API std::uint64_t sceneInstanceDrawCount() const;
      [[nodiscard]] VVE_V5_API std::uint32_t sceneVertexCount() const;
      [[nodiscard]] VVE_V5_API std::uint32_t sceneIndexCount() const;
      [[nodiscard]] VVE_V5_API std::size_t sceneDebugSampleCount() const;
      [[nodiscard]] VVE_V5_API std::optional<SceneDebugSample> sceneDebugSample(std::size_t index) const;
      [[nodiscard]] VVE_V5_API PixelExtent sceneShadowExtent() const;
      [[nodiscard]] VVE_V5_API std::optional<float> sceneShadowDepth(std::uint32_t x, std::uint32_t y) const;
      [[nodiscard]] VVE_V5_API std::optional<float> sceneSpotShadowDepth(std::uint32_t x, std::uint32_t y) const;
      [[nodiscard]] VVE_V5_API std::optional<float> scenePointShadowDepth(std::uint32_t face,
                                                                          std::uint32_t x,
                                                                          std::uint32_t y) const;
      [[nodiscard]] VVE_V5_API std::array<float, 4> lastClearColor() const;
...
export namespace vve::v5::vh {
   ...
   VVE_V5_API std::expected<void, Error> FrameHost::prepare(WindowSystem &windows) {
   ...
   VVE_V5_API std::expected<void, Error> FrameHost::renderClear(const std::array<float, 4> &color) {
      if (!ready()) { return {}; }
   ...
   VVE_V5_API FrameHost::renderTriangle(const std::array<float, 4> &color, std::span<const std::uint32_t> vertex_spirv,
                             std::string_view vertex_entry, std::span<const std::uint32_t> fragment_spirv,
                             std::string_view fragment_entry) {
      if (!ready()) { return {}; }
      const auto clear = vk::ClearColorValue{color};
   ...
   std::expected<void, Error>
   VVE_V5_API FrameHost::renderScene(const std::array<float, 4> &color, std::span<const std::uint32_t> vertex_spirv,
                          std::string_view vertex_entry, std::span<const std::uint32_t> fragment_spirv,
                          std::string_view fragment_entry, std::span<const std::uint32_t> shadow_vertex_spirv,
	                          std::string_view shadow_vertex_entry,
	                          std::span<const std::uint32_t> spot_shadow_vertex_spirv,
   ...
   VVE_V5_API std::size_t FrameHost::targetCount() const { return targets_.size(); }

   /// @brief Reports whether at least one drawable Vulkan target exists.
   VVE_V5_API bool FrameHost::ready() const {
   ...
   VVE_V5_API std::uint64_t FrameHost::presentedFrameCount() const { return presented_frames_; }

   /// @brief Returns how many hardcoded triangle draw calls completed.
   VVE_V5_API std::uint64_t FrameHost::triangleDrawCount() const { return triangle_draws_; }

   /// @brief Returns the hardcoded triangle vertex count.
   VVE_V5_API std::uint32_t FrameHost::triangleVertexCount() const { return 3; }

   /// @brief Returns how many scene buffer uploads completed.
   VVE_V5_API std::uint64_t FrameHost::sceneUploadCount() const { return scene_uploads_; }

   /// @brief Returns how many mesh draws were represented by the last scene draw path.
   VVE_V5_API std::uint64_t FrameHost::sceneMeshDrawCount() const { return scene_mesh_draws_; }

   /// @brief Returns how many instance draws were represented by the last scene draw path.
   VVE_V5_API std::uint64_t FrameHost::sceneInstanceDrawCount() const { return scene_instance_draws_; }

   /// @brief Returns the last uploaded scene vertex count.
   VVE_V5_API std::uint32_t FrameHost::sceneVertexCount() const { return scene_vertex_count_; }

   /// @brief Returns the last uploaded scene index count.
   VVE_V5_API std::uint32_t FrameHost::sceneIndexCount() const { return scene_index_count_; }

   /// @brief Returns how many GPU scene-debug sample slots contain data.
   VVE_V5_API std::size_t FrameHost::sceneDebugSampleCount() const {
   ...
   VVE_V5_API std::optional<SceneDebugSample> FrameHost::sceneDebugSample(std::size_t index) const {
   ...
   VVE_V5_API PixelExtent FrameHost::sceneShadowExtent() const { return PixelExtent{.width = 1024, .height = 1024}; }

   /// @brief Returns one copied shadow depth value if the latest pass produced it.
   VVE_V5_API std::optional<float> FrameHost::sceneShadowDepth(std::uint32_t x, std::uint32_t y) const {
   ...
   /// @brief Returns one copied spot shadow depth value if the latest pass produced it.
	   VVE_V5_API std::optional<float> FrameHost::sceneSpotShadowDepth(std::uint32_t x, std::uint32_t y) const {
	      const auto extent = sceneShadowExtent();
	      if (x >= extent.width || y >= extent.height) { return {}; }
	      const auto index = static_cast<std::size_t>(y) * extent.width + x;
	   ...
	   VVE_V5_API std::optional<float> FrameHost::scenePointShadowDepth(std::uint32_t face, std::uint32_t x, std::uint32_t y) const {
	      const auto extent = sceneShadowExtent();
   ...
   VVE_V5_API std::array<float, 4> FrameHost::lastClearColor() const { return last_clear_color_; }
   ...
   std::expected<void, Error> FrameHost::createInstance() {
      ...
      const auto result = low::createInstance("ViennaVulkanEngine v5", extensions, &raw_instance);
   ...
   std::expected<void, Error> FrameHost::createSceneShadowTarget(FrameTarget &target) {
      ...
      result = low::createShadowDepthTarget(*physical_device_, *device_, vk_extent,
                                            &raw_image, &raw_memory, &raw_view);
      if (result != vk::Result::eSuccess) { return std::unexpected(Error::platform_error); }
      target.spot_shadow_depth_memory = vk::raii::DeviceMemory{device_, static_cast<VkDeviceMemory>(raw_memory)};
      target.spot_shadow_depth_image = vk::raii::Image{device_, static_cast<VkImage>(raw_image)};
      target.spot_shadow_depth_view = vk::raii::ImageView{device_, static_cast<VkImageView>(raw_view)};
      target.spot_shadow_depth_layout = vk::ImageLayout::eUndefined;

      scene_spot_shadow_depths_.assign(static_cast<std::size_t>(extent.width) * extent.height, 1.0F);
   ...
   FrameHost::drawSceneShadowTarget(FrameTarget &target, std::span<const float> scene_constants) {
      ...
      result = low::recordSceneShadowDepth(*device_, *command_pool_, *command_buffer_,
                                           *target.spot_shadow_depth_image, *target.spot_shadow_depth_view,
                                           vk::Extent2D{extent.width, extent.height},
                                           target.spot_shadow_depth_layout, *target.spot_shadow_layout,
                                           *target.spot_shadow_pipeline, *target.scene_vertices,
                                           *target.scene_indices, target.scene_index_count,
                                           scene_constants, *target.spot_shadow_readback);
      if (result != vk::Result::eSuccess) { return std::unexpected(Error::platform_error); }
