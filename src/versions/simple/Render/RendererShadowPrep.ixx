...
/**
	...
	*
	* Functional objects:
	* - ForwardRendererShadowFrame carries the packed light arrays and every shadow view-projection matrix of one frame.
	* - ForwardRendererShadowPrep packs enabled spot, point, and directional lights and builds their shadow matrices.
	*/
export namespace vve::simple {
	...
	struct ForwardRendererShadowPrep {
		...
		[[nodiscard]] ForwardRendererShadowFrame prepareShadowFrame(const Mat4 &cameraView, Scalar cameraVerticalFov, Scalar cameraAspect, Scalar cameraNear, Scalar cameraFar) {
			...
			const auto addMeta = [&renderer](std::uint32_t type, std::size_t packedIndex, std::uint32_t firstLayer, const Mat4 &view, const Mat4 &projection, Scalar nearPlane, Scalar farPlane, Scalar bias) {
				renderer.shadowLightMeta.push_back({.light_index = static_cast<std::uint32_t>(packedIndex), .light_type = type, .shadow_slot = static_cast<std::uint32_t>(packedIndex),
			...
			for (const SpotLight &spot : renderer.scene.spotLights | std::views::take(kMaxShadowedSpotLights)) {
			...
			for (const PointLight &point : renderer.scene.pointLights | std::views::take(kMaxShadowedPointLights)) {
