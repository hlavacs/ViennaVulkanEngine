...
export namespace vve::v4::detail {
   ...
   template <typename THandle> [[nodiscard]] inline std::string jsonHandleId(THandle handle) {
      return std::to_string(handle.value);
   }

   /// @brief Writes a UTF-8 JSON string to disk, creating the parent directory when needed.
   [[nodiscard]] inline std::expected<void, Error> writeJsonFile(const std::filesystem::path &path,
