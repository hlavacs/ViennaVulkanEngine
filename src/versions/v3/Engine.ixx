...
namespace vve::v3 {
   ...
   namespace detail {
      ...
      invokeUserSystemInit(TSystem &system, vve::World &world) {
      ...
      template <typename TSystem>
      [[nodiscard]] std::expected<void, vve::Error>
      invokeUserSystemUpdate(TSystem &system, vve::World &world,
                             const FrameContext &frame_context,
                             const WindowFrameData &window_frame) {
         if constexpr (requires {
                          system.update(world, frame_context, window_frame);
                       }) {
            return system.update(world, frame_context, window_frame);
         } else if constexpr (requires {
                                 system.update(world, frame_context);
                              }) {
      ...
      [[nodiscard]] std::expected<void, vve::Error>
      initUserSystems(std::tuple<TSystems...> &systems, vve::World &world) {
         std::expected<void, vve::Error> result{};
         std::apply(
             [&](auto &...system) {
                ((result = invokeUserSystemInit(system, world),
                  result ? void() : void()),
                 ...);
             },
             systems);
      ...
      template <typename... TSystems>
      [[nodiscard]] std::expected<void, vve::Error>
      updateUserSystems(std::tuple<TSystems...> &systems, vve::World &world,
                        const FrameContext &frame_context,
                        const WindowFrameData &window_frame) {
         std::expected<void, vve::Error> result{};
         std::apply(
             [&](auto &...system) {
                ((result = invokeUserSystemUpdate(system, world, frame_context,
                                                  window_frame),
                  result ? void() : void()),
                 ...);
             },
   ...
   executeTaskGraph(const TaskGraph &task_graph,
      ...
      std::vector<std::size_t> remaining_dependencies(task_graph.nodes.size(),
                                                      0);
      for (std::size_t index = 0; index < task_graph.nodes.size(); ++index) {
         ...
         for (const auto &dependency : node.depends_on) {
            const auto dependency_it =
                node_indices.find(dependency.value.value());
      ...
      while (!ready_nodes.empty()) {
         ...
         if (node.callback) {
            if (auto callback_result = node.callback(execution_context);
                !callback_result) {
   ...
   public:
      ...
      [[nodiscard]] std::expected<int, vve::Error>
   ...
   BasicEngineImplementation<TUserSystems...>::BasicEngineImplementation(
      ...
      if (const auto enable_validation =
              config.tryGet<vve::EnableValidation>()) {
         validation_enabled_ = enable_validation->value;
      }

      if (const auto graphics_api =
              config.tryGet<vve::PreferredGraphicsApi>()) {
      ...
      if constexpr (sizeof...(TUserSystems) > 0) {
         if (const auto user_systems =
                 config.tryGet<vve::UserSystems<TUserSystems...>>()) {
   ...
   BasicEngineImplementation<TUserSystems...>::init() {
      if (*isInitialized()) {
      ...
      if (auto backend_result = runtime_.graphics_backend.init();
          !backend_result) {
      ...
      if (runtime_.gui_system != nullptr) {
         if (auto gui_result =
                 runtime_.gui_system->init(runtime_.graphics_backend);
             !gui_result) {
            return gui_result;
         }
      }
      ...
      if (auto user_system_result =
              detail::initUserSystems(user_systems_, world);
          !user_system_result) {
   ...
   template <typename... TUserSystems>
   std::expected<void, vve::Error>
   BasicEngineImplementation<TUserSystems...>::run() {
      if (!*isInitialized()) {
   ...
   template <typename... TUserSystems>
   std::expected<vve::FrameStatus, vve::Error>
   BasicEngineImplementation<TUserSystems...>::step() {
      if (!*isInitialized()) {
      ...
      const FrameContext frame_context{.frame_index = frame_index_++,
      ...
      if (auto execute_result =
              executeTaskGraph(*task_graph_, execution_context);
          !execute_result) {
      ...
      if (auto user_system_result = detail::updateUserSystems(
              user_systems_, world, frame_context, *runtime_.window_frame);
          !user_system_result) {
   ...
   template <typename... TUserSystems>
   std::expected<bool, vve::Error>
   BasicEngineImplementation<TUserSystems...>::isInitialized() const noexcept {
      return initialized_;
   }

   template <typename... TUserSystems>
   std::expected<int, vve::Error>
   BasicEngineImplementation<TUserSystems...>::getVersionMajor()
       const noexcept {
      return 3;
   }
   ...
   template <typename... TUserSystems>
   std::expected<void, vve::Error>
   BasicEngineImplementation<TUserSystems...>::loadFile(
   ...
   template <typename... TUserSystems>
   std::expected<void, vve::Error>
   BasicEngineImplementation<TUserSystems...>::rebuildTaskGraph() {
      if (!scene_) {
         return std::unexpected(vve::Error::invalid_argument);
      }
      ...
      auto task_graph = runtime_.task_graph_system.build(
          *scene_, task_systems, runtime_.window_system,
          runtime_.graphics_backend, runtime_.resource_system,
