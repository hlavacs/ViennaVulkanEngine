...
namespace vve::v3 {
   ...
   public:
      ...
      registerImportedScene(const ImportedScene &scene,
         ...
         records_.push_back(
             ResourceRecord{.id = scene.handle.value,
         ...
         for (const auto &mesh : scene.meshes) {
            records_.push_back(
                ResourceRecord{.id = mesh.handle.value,
         ...
         for (const auto &material : scene.materials) {
            records_.push_back(
                ResourceRecord{.id = material.handle.value,
      ...
      [[nodiscard]] std::expected<void, vve::Error>
      uploadResources(const FrameContext &, const SceneData &) {
      ...
      void registerTasks(TaskGraphBuilder &builder, const SceneData &) {
         ...
         builder.setTaskCallback(
             ...
             [this](const TaskExecutionContext &execution_context)
                ...
                return uploadResources(*execution_context.frame_context,
   ...
   ResourceSystemFacade<
       DefaultResourceSystemImplementation>::ResourceSystemFacade()
       : implementation_(
   ...
   std::string_view
   ResourceSystemFacade<DefaultResourceSystemImplementation>::name()
       const noexcept {
   ...
   ResourceSystemFacade<DefaultResourceSystemImplementation>::enumerate()
       const {
   ...
   template <>
   std::expected<void, vve::Error>
   ResourceSystemFacade<DefaultResourceSystemImplementation>::uploadResources(
   ...
   template <>
   void
   ResourceSystemFacade<DefaultResourceSystemImplementation>::registerTasks(
