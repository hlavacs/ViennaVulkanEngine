...
namespace vve::v3 {
   ...
   public:
      ...
      importScene(const std::filesystem::path &source_path) {
         ImportedScene scene{};
         scene.handle =
             SceneHandle{detail::makeStableHandle(source_path.string())};
   ...
   std::string_view
   AssetSystemFacade<AssimpAssetSystemImplementation>::name() const noexcept {
      return implementation_->name();
   }

   template <>
   std::expected<ImportedScene, vve::Error>
   AssetSystemFacade<AssimpAssetSystemImplementation>::importScene(
