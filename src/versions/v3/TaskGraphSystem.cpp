...
namespace vve::v3 {
   ...
   public:
      ...
      build(const SceneData &scene, std::span<ITaskSystem *const> task_systems,
         ...
         const auto poll_window_events =
             TaskGraphBuilder::taskHandleFor("task.poll_window_events");
         const auto end_frame =
             TaskGraphBuilder::taskHandleFor("task.end_frame");
         ...
         for (const auto &leaf : builder.leafTasks()) {
            if (leaf.value == begin_frame.value ||
                leaf.value == end_frame.value) {
               continue;
            }
         ...
         const auto update_transforms =
             builder.findTask("task.update_transforms");
         ...
         if (const auto end_frame_task = builder.findTask("task.end_frame")) {
            for (const auto &pipeline : render_pipelines) {
               builder.addDependency(
                   *end_frame_task, TaskGraphBuilder::taskHandleFor(std::format(
                                        "task.window.{}.consume_frame_output",
                                        pipeline.window_id)));
            }
         }
   ...
   TaskGraphSystemFacade<
       DefaultTaskGraphSystemImplementation>::TaskGraphSystemFacade()
       : implementation_(
   ...
   std::string_view
   TaskGraphSystemFacade<DefaultTaskGraphSystemImplementation>::name()
       const noexcept {
   ...
   TaskGraph TaskGraphSystemFacade<DefaultTaskGraphSystemImplementation>::build(
       const SceneData &scene, std::span<ITaskSystem *const> task_systems,
       WindowSystem &window_system, GraphicsBackend &graphics_backend,
       ResourceSystem &resource_system, SceneSystem &scene_system,
       RenderSystem &render_system,
       std::span<const WindowRenderPipeline> render_pipelines) {
      return implementation_->build(
          scene, task_systems, window_system, graphics_backend, resource_system,
          scene_system, render_system, render_pipelines);
   }
