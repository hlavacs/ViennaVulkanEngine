...
namespace vve {
    VkShaderModule PiplineRasterized::createShaderModule(const std::vector<char>& code, VkDevice& device) {
    ...
    void PiplineRasterized::createRenderPass() {
    ...
    void PiplineRasterized::createFramebuffers() {
    ...
    PiplineRasterized::~PiplineRasterized() {
    }

    void PiplineRasterized::freeResources() {
    ...
    void PiplineRasterized::bindRenderTarget(RenderTarget* target) {
        renderTargets.push_back(target);
    }

    void PiplineRasterized::bindDepthRenderTarget(RenderTarget* target) {
        depthTarget = target;
    }

    void PiplineRasterized::recreateFrameBuffers(VkExtent2D extent) {
        this->extent = extent;
        for (VkFramebuffer frameBuffer : framebuffers) {
            vkDestroyFramebuffer(device, frameBuffer, nullptr);
        }
    ...
    void PiplineRasterized::initGraphicsPipeline() {
        createRenderPass();
        createFramebuffers();
    ...
    void PiplineRasterized::recordCommandBuffer(uint32_t currentFrame) {
