...
const std::vector<const char*> requiredDeviceExtensions = { VK_KHR_SWAPCHAIN_EXTENSION_NAME, VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME, VK_KHR_MAINTENANCE_4_EXTENSION_NAME };
...
void EndSingleTimeCommand(VkCommandBuffer commandBuffer) {
    ...
    VkSubmitInfo submitInfo{ .sType = VK_STRUCTURE_TYPE_SUBMIT_INFO, .waitSemaphoreCount = 0, .commandBufferCount = 1
        , .pCommandBuffers = &commandBuffer, .signalSemaphoreCount = 0 };
...
auto InitVulkan() {
    ...
    }
    {
        ...
        for (const auto& pdev : physicalDevices) {
            ...
            vkEnumerateDeviceExtensionProperties(pdev, nullptr, &count, deviceExtensions.data());
            std::set<std::string> required{ requiredDeviceExtensions.begin(), requiredDeviceExtensions.end() };
            for (const auto& ext : deviceExtensions) { required.erase(ext.extensionName); }
            bool suitable = !required.size();

            VkPhysicalDeviceProperties deviceProperties;
            vkGetPhysicalDeviceProperties(pdev, &deviceProperties);
            if (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU && suitable) { physicalDevice = pdev; break; }
            if (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU && suitable) { physicalDevice = pdev; }
        }
    ...
    }
    {
        ...
        VkDeviceCreateInfo deviceCreateInfo{ .sType = VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO, .pNext = nullptr
            ...
            , .enabledExtensionCount = (uint32_t)requiredDeviceExtensions.size(), .ppEnabledExtensionNames = requiredDeviceExtensions.data()
